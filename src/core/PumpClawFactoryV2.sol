...
/// @title PumpClawFactoryV2
/// @notice Fair launch tokens with configurable initial FDV
/// @dev Uses FDV to calculate initial price, ensuring fairer distribution
contract PumpClawFactoryV2 is ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public constant TOKEN_SUPPLY = 1_000_000_000e18; // Fixed 1B supply
    uint256 public constant MIN_ETH = 0.05 ether; // Higher minimum for better liquidity
    uint256 public constant DEFAULT_FDV = 30 ether; // Default initial FDV
    uint256 public constant MIN_FDV = 10 ether; // Minimum FDV
    uint256 public constant MAX_FDV = 1000 ether; // Maximum FDV
    ...
    /// @param initialFdv Initial fully diluted valuation in ETH (e.g., 20 ether = $60k at $3k ETH)
    ...
    function _createToken(
        ...
        require(msg.value >= MIN_ETH, "ETH below minimum (0.05 ETH)");
        require(initialFdv >= MIN_FDV, "FDV below minimum (10 ETH)");
        require(initialFdv <= MAX_FDV, "FDV above maximum (1000 ETH)");
        ...
        // Calculate sqrt price based on FDV (not actual ETH deposited)
        // Price = FDV / TOKEN_SUPPLY = ETH per token
        uint160 sqrtPriceX96;
        if (tokenIsToken0) {
            // price = WETH / TOKEN = FDV / SUPPLY
            sqrtPriceX96 = _calculateSqrtPrice(initialFdv, TOKEN_SUPPLY);
        } else {
            // price = TOKEN / WETH = SUPPLY / FDV
        ...
        // Calculate liquidity from actual amounts
